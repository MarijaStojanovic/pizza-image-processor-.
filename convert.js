...
module.exports.convert= (bucket, filePath) => {
  ...
  .then(filePath => {
    return new Promise((resolve, reject) => {
      const localFilePath = path.join('/tmp/images/', fileName);
      exec(`convert ${localFilePath} -resize 1024x1024\\> ${localFilePath}`,
      (err, stdout, stderr) => {
        if (err) {
          return reject(err);
        }
  ...
  .then(filePath => {
    return new Promise((resolve, reject) => {
      ...
      exec(`convert ${localFilePath} -resize 120x120\\> ${localThumbnailPath}`,
      (err, stdout, stderr) => {
        if (err) {
          return reject(err);
        }
